...
### Added
- **Markdown 渲染全覆盖 + md 只读预览档**：新增 `public/md.js` 渲染层，把以前直接掉成纯文本的语法补齐——GFM 表格/任务列表/删除线之外，还有 front matter（折叠成元信息表，不再被当成分隔线+标题的乱码）、GFM 提示块 `> [!NOTE|TIP|IMPORTANT|WARNING|CAUTION]`、脚注 `[^x]`（带编号与回跳）、`==高亮==`/上标/下标/`<kbd>`、KaTeX 数学（`$…$`、`$$…$$`、`\(…\)`、`\[…\]`，`$12 和 $20` 这种价格不会被误吃）、```mermaid 图（3MB 的 mermaid 只在文档里真有图时才懒加载）、标题锚点。md 里的相对图片/链接按文件所在目录解析：图片不再一律裂图，链接指向本地文件时点一下就在翻箱里打开。渲染结果统一过 DOMPurify——预览陌生 md 时不给它碰文件系统 API 的机会
- **md 编辑器多一档「预览」**：编辑条改成 `预览 / 富文本 / 源码` 三档。含富文本无法无损往返的语法（脚注、数学等）的文件以前会被直接锁进 Monaco 源码，看上去就是「markdown 没渲染」——现在默认落到只读预览，要改再点「源码」，无损保护照旧
- **图片 lightbox 重做**：滚轮以光标为锚点缩放、拖拽平移、双击 1:1、`←/→` 翻上一张下一张（当前目录的图片自动成组，md 文档里的图按文档内顺序成组）、底部工具条显示缩放比例/文件名/第几张，`+ - 0 1 Esc` 全套快捷键
...
### Changed
- 顶栏「跳转到路径」按钮重新设计：从一个看不出含义的「文件夹+箭头」方块，改成命令提示符形状的胶囊（`› 跳转`），顶栏挤窄时收起文字只留 `›`
